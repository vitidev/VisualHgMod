        ...
        static public Process DiffDialog(string sccFile, string file, string commandMask)
        {
          ...
          if(root != String.Empty)
          {
            ...
            else
            {
                ...
                cmd = PrepareDiffCommand(versionedFile, currentFile, commandMask); 
                return InvokeCommand(HG.GetTortoiseHGDirectory() + "lib\\kdiff3.exe", root, cmd);
            }
          }
          return null;
        }

        public static string GetLatestFileRevisionToTemp(string sccFile, string file)
        {
            String root = HGLib.HG.FindRootDirectory(file);
            if (root != String.Empty)
            {
                ...
                string extension = Path.GetExtension(sccFile);
                string versionedFile = tmpFile + "(base)" + extension;
